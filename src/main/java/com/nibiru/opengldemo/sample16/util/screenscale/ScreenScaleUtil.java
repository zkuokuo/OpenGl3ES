...
public class ScreenScaleUtil {
    ...
    public static ScreenScaleResult calScale
        ...
        ScreenScaleResult.ScreenOrien so = null;

        //首先判断目标是横屏还是竖屏
        if (targetWidth > targetHeight) {
            so = ScreenScaleResult.ScreenOrien.HP;
        } else {
            so = ScreenScaleResult.ScreenOrien.SP;
        }
        ...
        //进行横屏结果的计算
        if (so == ScreenScaleResult.ScreenOrien.HP) {
            //计算目标的宽高比
        ...
        //进行竖屏结果的计算
        if (so == ScreenScaleResult.ScreenOrien.SP) {
            //计算目标的宽高比
            float targetRatio = targetWidth / targetHeight;
