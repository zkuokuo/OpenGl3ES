...
import com.nibiru.opengldemo.sample16.object.LoadedObjectVertexNormalTexture;
import com.nibiru.opengldemo.sample16.util.DrawLine;
...
public class CollectionView extends BNAbstractView {
    ...
    public void initDoll()//初始化娃娃的相关参数
    {
        ...
        numberaward.add("012");
        dollobj.add(1, doll0);
        textureId.add(doll0Id);
        objscale.add(2f);
        numberaward.add("345");
        dollobj.add(2, doll2);
        textureId.add(doll2Id);
        objscale.add(2f);
        numberaward.add("678");
        dollobj.add(3, ParrotMd);
        textureId.add(parrotId);
        objscale.add(8f);
        numberaward.add("036");
        dollobj.add(4, RobotMD);
        textureId.add(robotId);
        objscale.add(9f);
        numberaward.add("147");
        dollobj.add(5, CarMD);
        textureId.add(CarId);
        objscale.add(8f);
        numberaward.add("258");
        dollobj.add(6, tvmodle);
        textureId.add(tvId);
        objscale.add(2f);
        dollobj.add(7, doll1);
        textureId.add(doll1Id);
        objscale.add(2f);
        dollobj.add(8, Camera);
        textureId.add(CameraId);
        objscale.add(7.5f);
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                if (x > back_left && x < back_right && y > back_top && y < back_bottom && !isSale) {//点击返回按钮
                    if (!effictOff) {
                ...
                for (int i = 0; i < 9; i++) {
                    if (x > numberlocationdata[i][0] - 90 && x < numberlocationdata[i][0] + 140
                            && y > numberlocationdata[i][1] - 300 && y < numberlocationdata[i][1] - 60 && !isSale) {//点击娃娃进入出售界面
                        if (i == 4 && islock) {
                            break;
                        } else {
                            isOntouch[i] = true;
